...
#include <iostream>		//	Standard library headers for I/O, strings, containers, and stream manipulation.
#include <string>
#include <map>
#include <vector>
#include <sstream>
...
//	A struct to hold the configuration for a specific `location` block.
...
//	A struct to hold the configuration for an entire `server` block.
...
struct	ServerConfig
{
	...
	bool						enable_reuse_addr;		//	A to allow (or not) the port to be reused quickly after a program exits.
